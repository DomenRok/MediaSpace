...
class Afterglow {
	private lightboxtriggers: any[];
	private players: any[];
	private eventbus: boolean|Eventbus;
	private afterinit: any[];
	 ...
	 */
	on(playerid: any, eventname: any, _callback: any){
		if(!this.eventbus){
			this.afterinit.push(() => {
			...
			(this.eventbus as any).subscribe(playerid, eventname, _callback);
		}
	}
	 ...
	 * @param      mixed 	_callback  The callback
	 */
	off(playerid: any, eventname: any, _callback: any){
		if(!this.eventbus){
			this.afterinit.push(() => {
			...
			(this.eventbus as any).unsubscribe(playerid, eventname, _callback);
		}
	}
	 ...
	 * @return void
	 */
	bindLightboxTriggerEvents(trigger: any){
		trigger.on('trigger',() => {
